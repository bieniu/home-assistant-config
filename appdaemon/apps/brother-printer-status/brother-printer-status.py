...
class BrotherPrinterStatus(hass.Hass):
    def initialize(self):
        ...
        try:
            ...
            self.error(
                "Wrong arguments! You must supply a valid printer "
        ...
        try:
            ...
            self.error(
                "Wrong arguments! Argument status_interval has to be " "an integer."
            )
            return
        try:
            ...
            self.error(
                "Wrong arguments! Argument info_interval has to be " "an integer."
            )
            return
    ...
    def update_printer_status_page(self, kwargs):
        page = self.download_page("http://{}{}".format(self.host, self.STATUS_URL))
            ...
            if regex_res:
                ...
                if status:
                    ...
                    if self.use_mqtt:
                        ...
                        self.mqtt_publish(topic, payload, entity, sensor, status)
                    else:
                        attributes = {
            ...
            if regex_res:
                ...
                if counter:
                    ...
                    if self.use_mqtt:
                        ...
                        self.mqtt_publish(topic, payload, entity, sensor, counter)
                    else:
                        attributes = {
                            "friendly_name": "Printer " + sensor,
                        ...
                        self.set_state(entity, state=counter, attributes=attributes)
            ...
            if regex_res:
                ...
                if drum_usage:
                    ...
                    if self.use_mqtt:
                        ...
                        self.mqtt_publish(topic, payload, entity, sensor, drum_usage)
                    else:
                        attributes = {
                        ...
                        self.set_state(entity, state=drum_usage, attributes=attributes)
    ...
    def mqtt_publish(self, topic, payload, entity, sensor, new_state):
        if self.mac_simple and not self.entity_exists(entity):
            self.call_service(
                "mqtt/publish",
                topic="homeassistant/sensor/" + topic + "config",
